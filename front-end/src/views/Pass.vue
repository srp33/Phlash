<template>
  <div class="container">
    ...
    <div class="alert alert-primary">
      ...
      <button class="btn btn-light" type="button" @click="deleteCDS($route.params.cdsID)" v-if="!blastResultsExist">
    ...
    <div class="info-bottom">
      ...
      <button type="button" class="btn btn-light" @click="deleteCDS($route.params.cdsID)" v-if="!blastResultsExist">
        <strong>Delete {{ $route.params.cdsID }}</strong>
      </button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    getData(cdsID) {
      axios.get(`http://localhost:5000/api/annotations/pass/${this.$route.params.phageID}/${cdsID}`)
        .then(response => {
    ...
    updateCDS(payload, cdsID) {
      axios.put(`http://localhost:5000/api/annotations/pass/${this.$route.params.phageID}/${cdsID}`,
          payload
        )
        .then(() => {
    ...
    deleteCDS(cdsID) {
      axios.delete(`http://localhost:5000/api/annotations/pass/${this.$route.params.phageID}/${cdsID}`)
        .then(() => {
          this.$router.push(`/annotations/${this.$route.params.phageID}`);
        })
        .catch(error => {
