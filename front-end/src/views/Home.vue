    ...
    <div class="container">
      ...
      <div class="alert alert-primary" align="left">
        ...
        <div class="alert alert-warning" v-if="idStatus !== ''">
          ...
          You have until <strong>{{ dateToBeDeleted }}</strong> to complete annotations for this phage.
        </div>
        ...
        <div class="nav-btns-wrapper">
          <router-link :to="{ name: 'Upload', params: {phageID: phageID} }"
            v-if="(idStatus.includes('ID created')) || 
    ...
    checkPhageID(phageID) {
      axios.post(process.env.VUE_APP_BASE_URL + `/home/${phageID}`)
        .then(response => {
          ...
          this.dateToBeDeleted = `${monthNames[date.getUTCMonth()]} ${date.getUTCDate()}, ${date.getUTCFullYear()}`
        })
        .catch(error => {
          console.error(error);
        });
