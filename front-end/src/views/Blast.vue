<template>
  <div class="wrapper" @click="checkFiles">
    ...
    <div class="container">
      <h1>BLAST</h1>
        <div v-if="downloadLoading" class="alert alert-warning">
        <strong>PLEASE DO NOT REFRESH OR CLOSE OUT OF THIS PAGE.<br />
        The Blast input files are being created which may take several minutes.
        </strong>
      </div>
      <div v-else>
          ...
          <ol>
            ...
            <li class="step">
              ...
              <div class="alert alert-warning" v-if="clickedNCBI">
                ...
                time, it is still probably working correctly. If an error occurs
                refresh the page or 
                try re-running that file. Oftentimes it will then work correctly. 
        ...
        <div class="nav-btns-wrapper">
          ...
          <router-link
            ...
            :event="blastDownloaded && blastUploaded ? 'click' : ''"
          >
            <button class="btn btn-light btn-nav disabled" id="next-bottom" @click="displayOutputFiles()">
              <strong>Next</strong>
              <svg
                class="bi bi-arrow-right"
                width="1em"
...
export default {
  ...
  methods: {
    ...
    setDropzone() {
      return {
        ...
        init: function() {
          axios
            .post(
              this.options.url.slice(0,this.options.url.indexOf("test")) + `numFiles/none`
            )
            .then((response) => {
          ...
          axios
            .post(
              this.options.url.slice(0,this.options.url.indexOf("test")) + `displayOutput/none`
            )
            .then((response) => {
              console.log(response.data);
          ...
          this.on("removedfile", function(file) {
            ...
            axios
              .post(
                this.options.url.slice(0,this.options.url.indexOf("test")) + `deleteOutput/${file.name}`
              )
              .then((response) => {
                console.log(response.data);
              })
     ...
     */
    getUploadUrl() {
      return `http://daniel.byu.edu:5000/phlash_api/blast/${this.$route.params.phageID}/test/${this.numFiles}`;
    },
